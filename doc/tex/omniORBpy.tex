...
The specifies two standard command line arguments which provide a
...
an action of `\code{unix,tcp,\dsc{}ssl}' means to use a Unix transport
if there is one, failing that a TCP transport, failing \emph{that} an
SSL transport. In the absence of any explicit rules, the client uses
the implicit rule of `\code{* unix,tcp,ssl}'.
