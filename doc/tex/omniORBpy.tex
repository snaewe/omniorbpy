...
\item Interceptors are not available to Python code.
...
with the \op{traceLevel}, \op{traceInvocations} and \op{traceThreadId}
functions in the \code{omniORB} module. Calling one of the functions
with no arguments returns the current value; calling it with a single
integer argument sets the value.
...
The C++ file \file{include/omniORB4/minorCode.h} contains definitions
...
To use the SSL transport from Python you must import the
\code{omniORB.sslTP} module before calling \op{CORBA.ORB\_init}.
